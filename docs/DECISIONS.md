...
- **Existing `messages.yml` files are never overwritten.** A key only falls back to the bundled text when it's missing entirely, so a changed default doesn't reach an existing file. Decided 2026-09-23: no upgrade handling before the first release, because there are no user files yet. During development, delete `test-server/plugins/Hounded/messages.yml` after changing message text. Before release, add a version number to the file, with a backup and a console warning when it's outdated.
- **Runner death = eliminated for the round.** Hunters win when every runner is eliminated.
- **Eliminated runners spectate** (decided 2026-09-23, not built yet). They switch to spectator on respawn and go back to survival when the round ends. A config toggle lets them keep playing instead.
- **Runners who quit get a grace period** (decided 2026-09-23, not built yet). There's a configurable rejoin window, default 5 minutes, where 0 means immediate. A runner not back in time is eliminated, as if they had died. The timing logic lives in `GameSession` with the injected `Clock`, so it's unit-tested.
